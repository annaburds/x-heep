...
void adaptive_avg_pool_1d(float* input, float* output, int n_channels, int input_size) {
    for (int c = 0; c < n_channels; c++) {
        float sum = 0.0;
        for (int i = 0; i < input_size; i++) {
            sum += input[c * input_size + i];
        }
        output[c] = sum / input_size;
    }
}
...
int16_t forward_propagation(int16_t *data, int16_t *intermediate) {
    ...
    //Early Exit Branch
    int16_t *input_ee = layer_out;
    int16_t *output_ee = layer_out;
    adaptive_avg_pool_1d(input_ee,output_ee,256*,256*128*64);
