...
void SL_DMA_SEND(uint32_t *src, uint32_t *dst, uint32_t large, uint32_t chunk);
...
int main(int argc, char *argv[]){
    ...
    for (uint32_t i = 0; i < chunks; i++) {
        ...
        SL_DMA_SEND(to_be_sent_4B + i * DMA_DATA_LARGE, addr_p, DMA_DATA_LARGE,i);
    }
    if (remainder > 0) {
        // SL_CPU_SEND(to_be_sent_4B + chunks * DMA_DATA_LARGE, addr_p + chunks * DMA_DATA_LARGE, remainder);
        SL_DMA_SEND(to_be_sent_4B + chunks * DMA_DATA_LARGE, addr_p + chunks * DMA_DATA_LARGE, remainder,remainder);
    }

    printf("Transmission succeded\n");  
    return EXIT_SUCCESS;
}
...
void __attribute__ ((optimize("00"))) SL_DMA_SEND(uint32_t *src, uint32_t *dst, uint32_t large, uint32_t chunk){
